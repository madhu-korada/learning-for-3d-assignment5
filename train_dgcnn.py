...
# from models_pointnet2 import Net as cls_model
from models_dgcnn import DGCNN_cls as cls_model #cls_model
...
def train(train_dataloader, model, opt, epoch, args, writer, debug=False):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if sample_random:
            idx = torch.randperm(point_clouds.shape[1])[:2000]
...
def test(test_dataloader, model, epoch, args, writer):
    ...
    # Evaluation in Segmentation Task
        ...
        for batch in test_dataloader:
            ...
            # ------ TO DO: Make Predictions ------
            with torch.no_grad():     
                pred_labels = model(point_clouds).argmax(dim=2, keepdim=False)
...
def create_parser():
    ...
    parser.add_argument('--exp_name', type=str, default="exp_dgcnn", help='The name of the experiment')
